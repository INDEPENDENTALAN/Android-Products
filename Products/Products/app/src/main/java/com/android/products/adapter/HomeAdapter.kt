...
class HomeAdapter(var activity: Activity,var arrayList: ArrayList<ProductsEntity>): RecyclerView.Adapter<HomeAdapter.HomeViewHolder>() {
    ...
    override fun onBindViewHolder(holder: HomeViewHolder, position: Int) {
        ...
        holder.imageView_home_image.setImageBitmap(onStringToBitmap(arrayList[position].image))
        holder.imageButton_home_edit.setOnClickListener { l: View? ->
            ...
            activity.startActivityForResult(
                ...
                "request",
                4
            ).putExtra("name", arrayList[position].name).putExtra("price", arrayList[position].price).putExtra("details", arrayList[position].details), 4)
